...
- Primary domain lives in `src/main/java/br/com/vsjr/labs/log`; `src/main/java/br/com/vsjr/labs/exemple` is usage/demo code.
...
- `interceptor/LogInterceptor` adds technical location to MDC and records Micrometer timers/counters.
- `interceptor/RastreamentoInterceptor` creates child spans and runs before `LogInterceptor` (`APPLICATION - 10`).
- `filtro/LogContextoFiltro` initializes request MDC (`traceId`, `spanId`, `userId`, `servico`) and clears MDC on response.
...
- Business methods annotated with `@Rastreado` create child span; same methods with `@Logged` collect class/method metrics.
...
- Observability config is in `src/main/resources/application.properties` (OTLP endpoint `http://localhost:4317`, metrics at `/q/metrics`).

## Testing patterns in this repo
- Unit tests are JUnit 5 with direct JBoss LogManager capture (`src/test/java/br/com/vsjr/labs/log/dsl/LogSistematicoTest.java`).
